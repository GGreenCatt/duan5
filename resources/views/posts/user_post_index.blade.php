{{-- TẠO FILE MỚI NÀY: resources/views/posts/user_post_index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4">
            ...
            <div class="mb-12 border-t border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center overflow-x-auto py-3 space-x-6">
                    ...
                    {{-- Link "Tất cả" trỏ đến route mới --}}
                    <a href="{{ route('user.posts.index') }}" class="flex-shrink-0 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors
                       {{ !isset($categoryName) ? 'text-indigo-600 dark:text-indigo-400 font-bold' : '' }}">
                        Tất cả
                    </a>
                    
                    @foreach($categories as $category)
                         {{-- Link danh mục trỏ đến route mới --}}
                        <a href="{{ route('user.posts.by_category', $category->id) }}" class="flex-shrink-0 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors
