{{-- TẠO FILE MỚI NÀY: resources/views/posts/user_post_list.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight mb-2 sm:mb-0">
                {{-- Hiển thị tiêu đề động, ví dụ: "Danh mục: Công nghệ" hoặc "Tất cả bài viết" --}}
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4">
            ...
            <div class="mb-12 border-t border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center overflow-x-auto py-3 space-x-6">
                    ...
                    {{-- Thêm link "Tất cả" --}}
                    <a href="{{ route('posts.list') }}" class="flex-shrink-0 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors
                       {{ !isset($categoryName) ? 'text-indigo-600 dark:text-indigo-400 font-bold' : '' }}">
                        Tất cả
                    </a>
                    
                    @foreach($categories as $category)
                        <a href="{{ route('posts.by_category', $category->id) }}" class="flex-shrink-0 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors
                           {{ (isset($categoryName) && $categoryName == $category->name) ? 'text-indigo-600 dark:text-indigo-400 font-bold' : '' }}"> {{-- Highlight danh mục đang chọn --}}
                            {{ $category->name }}
                        </a>
                    @endforeach
                </div>
            ...
            @endif
            {{-- =============== KẾT THÚC PHẦN: THANH DANH MỤC =============== --}}


            {{-- =============== BẮT ĐẦU PHẦN: LƯỚI BÀI VIẾT =============== --}}
            ...
            @else
                {{-- Thông báo khi không có bài viết --}}
                 <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg p-10 text-center">
                    ...
                    <p class="mt-2 text-gray-600 dark:text-gray-400">Không có bài viết nào phù hợp với danh mục này.</p>
                </div>
            @endif
            {{-- =============== KẾT THÚC PHẦN: LƯỚI BÀI VIẾT =============== --}}
